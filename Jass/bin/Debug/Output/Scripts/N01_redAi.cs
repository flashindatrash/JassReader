...
namespace Jass {
	...
	public class N01_redAi {
		...
		private void main() {//function main takes nothing returns nothing
			...
			CampaignDefenderEx(1,1,0, HEAD_HUNTER);//call CampaignDefenderEx( 1,1,0, HEAD_HUNTER	)
			...
			while (true) {//loop
				...
				InitAssaultGroup();//call InitAssaultGroup()
				CampaignAttackerEx(2,2,3, GRUNT);//call CampaignAttackerEx( 2,2,3, GRUNT		)
				CampaignAttackerEx(0,0,1, HEAD_HUNTER);//call CampaignAttackerEx( 0,0,1, HEAD_HUNTER	)
				SuicideOnPlayerEx(M6,M6,M5,user);//call SuicideOnPlayerEx(M6,M6,M5,user)
				//
				InitAssaultGroup();//call InitAssaultGroup()
				CampaignAttackerEx(4,4,6, GRUNT);//call CampaignAttackerEx( 4,4,6, GRUNT       )
				SuicideOnPlayerEx(M6,M6,M5,user);//call SuicideOnPlayerEx(M6,M6,M5,user)
			}//endloop
		}//endfunction
