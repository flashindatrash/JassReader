...
namespace Jass {
	...
	public class O05_redAi {
		...
		private void MikesSuicide(int easy, int med, int hard, player user) {//function MikesSuicide takes integer easy, integer med, integer hard, player user returns nothing
			...
			SuicideOnPlayerEx(easy/factor, med/factor, hard/factor, user);//call SuicideOnPlayerEx(easy/factor, med/factor, hard/factor, user)
		}//endfunction
		//
		//
		//
		private void main() {//function main takes nothing returns nothing
			...
			MikesSuicide(4*60,4*60,4*60,user);//call MikesSuicide(4*60,4*60,4*60,user)
			//
			InitAssaultGroup();//call InitAssaultGroup()
			CampaignAttacker(EASY, 5, ARCHER);//call CampaignAttacker( EASY, 5, ARCHER          )
			CampaignAttacker(EASY, 2, HUNTRESS);//call CampaignAttacker( EASY, 2, HUNTRESS        )
			MikesSuicide(6*60,6*60,6*60,user);//call MikesSuicide(6*60,6*60,6*60,user)
			//
			InitAssaultGroup();//call InitAssaultGroup()
			CampaignAttacker(EASY, 3, ARCHER);//call CampaignAttacker( EASY, 3, ARCHER      )
			CampaignAttacker(EASY, 1, HUNTRESS);//call CampaignAttacker( EASY, 1, HUNTRESS    )
			MikesSuicide(2*60,2*60,2*60,user);//call MikesSuicide(2*60,2*60,2*60,user)
			//
			InitAssaultGroup();//call InitAssaultGroup()
			...
			MikesSuicide(3*60,3*60,3*60,user);//call MikesSuicide(3*60,3*60,3*60,user)
			//
			InitAssaultGroup();//call InitAssaultGroup()
