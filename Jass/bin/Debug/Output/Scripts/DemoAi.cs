...
namespace Jass {
	...
	public class DemoAi {
		...
		private void paladin_sequence() {//function paladin_sequence takes nothing returns nothing
			...
			set_build(1, DemoAi.town_hall);//call set_build(1, town_hall )
			set_build(4, DemoAi.peon);//call set_build(4, peon      )
			set_build(7, DemoAi.peon);//call set_build(7, peon      )
			set_build(1, DemoAi.house);//call set_build(1, house     )
			set_build(9, DemoAi.peon);//call set_build(9, peon      )
			set_build(1, DemoAi.garrison);//call set_build(1, garrison  )
			set_build(1, DemoAi.paladin);//call set_build(1, paladin   )
			set_build(1, HumanAi.smith);//call set_build(1, smith     )
			set_build(1, DemoAi.footman);//call set_build(1, footman   )
			set_build(2, DemoAi.house);//call set_build(2, house     )
			set_upgrd(1, DemoAi.Uweapons);//call set_upgrd(1, Uweapons  )
			set_build(4, DemoAi.footman);//call set_build(4, footman   )
			set_upgrd(1, DemoAi.Uarmor);//call set_upgrd(1, Uarmor    )
			set_build(3, DemoAi.rifle);//call set_build(3, rifle     )
			//
			set_build(6, DemoAi.footman);//call set_build(6, footman   )
			set_build(4, DemoAi.rifle);//call set_build(4, rifle     )
			set_upgrd(2, DemoAi.Uweapons);//call set_upgrd(2, Uweapons  )
			set_build(6, DemoAi.rifle);//call set_build(6, rifle     )
			//
			set_build(1, HumanAi.mill);//call set_build(1, mill      )
			set_upgrd(2, DemoAi.Uarmor);//call set_upgrd(2, Uarmor    )
			set_build(3, DemoAi.house);//call set_build(3, house     )
			return;//return
			set_build(6, DemoAi.peon);//call set_build(6, peon      )
			set_build(1, DemoAi.house);//call set_build(1, house     )
			set_build(4, DemoAi.footman);//call set_build(4, footman   )
			//
			set_build(2, DemoAi.rifle);//call set_build(2, rifle     )
			set_build(1, HumanAi.keep);//call set_build(1, keep      )
			set_build(2, DemoAi.house);//call set_build(2, house     )
			set_build(5, DemoAi.footman);//call set_build(5, footman   )
			set_build(4, DemoAi.rifle);//call set_build(4, rifle     )
			//
			set_build(1, DemoAi.armory);//call set_build(1, armory    )
			set_upgrd(2, DemoAi.Uarmor);//call set_upgrd(2, Uarmor    )
			set_build(1, DemoAi.altar);//call set_build(1, altar     )
			set_upgrd(1, DemoAi.Uranged);//call set_upgrd(1, Uranged   )
			set_build(1, DemoAi.church);//call set_build(1, church    )
			set_upgrd(2, DemoAi.Uranged);//call set_upgrd(2, Uranged   )
			set_upgrd(3, DemoAi.Uweapons);//call set_upgrd(3, Uweapons  )
			set_build(1, DemoAi.priest);//call set_build(1, priest    )
			//
			set_build(6, DemoAi.footman);//call set_build(6, footman   )
			set_build(5, DemoAi.rifle);//call set_build(5, rifle     )
			set_build(2, DemoAi.priest);//call set_build(2, priest    )
			//
			set_build(3, DemoAi.priest);//call set_build(3, priest    )
			set_build(6, DemoAi.rifle);//call set_build(6, rifle     )
			set_build(3, DemoAi.house);//call set_build(3, house     )
			//
			set_upgrd(3, DemoAi.Uranged);//call set_upgrd(3, Uranged   )
			set_upgrd(3, DemoAi.Uarmor);//call set_upgrd(3, Uarmor    )
			set_build(1, HumanAi.castle);//call set_build(1, castle    )
			set_build(7, DemoAi.rifle);//call set_build(7, rifle     )
			set_build(4, DemoAi.priest);//call set_build(4, priest    )
			set_build(8, DemoAi.rifle);//call set_build(8, rifle     )
			set_build(2, DemoAi.knight);//call set_build(2, knight    )
			set_build(4, DemoAi.house);//call set_build(4, house     )
			set_build(1, HumanAi.sanctum);//call set_build(1, sanctum   )
			set_build(4, DemoAi.knight);//call set_build(4, knight    )
			set_build(5, DemoAi.house);//call set_build(5, house     )
			set_build(1, DemoAi.caster);//call set_build(1, caster    )
			set_build(5, DemoAi.knight);//call set_build(5, knight    )
			set_build(2, DemoAi.caster);//call set_build(2, caster    )
			set_build(9, DemoAi.rifle);//call set_build(9, rifle     )
			set_build(3, DemoAi.caster);//call set_build(3, caster    )
			set_build(6, DemoAi.house);//call set_build(6, house     )
			set_build(11, DemoAi.rifle);//call set_build(11, rifle    )
			set_build(5, DemoAi.priest);//call set_build(5, priest    )
			set_upgrd(1, DemoAi.Ubldgs);//call set_upgrd(1, Ubldgs    )
			set_upgrd(1, DemoAi.Uartillery);//call set_upgrd(1, Uartillery)
			set_upgrd(2, DemoAi.Uartillery);//call set_upgrd(2, Uartillery)
			set_upgrd(3, DemoAi.Uartillery);//call set_upgrd(3, Uartillery)
			set_upgrd(2, DemoAi.Ubldgs);//call set_upgrd(2, Ubldgs    )
			set_upgrd(3, DemoAi.Ubldgs);//call set_upgrd(3, Ubldgs    )
		...
		private void paladin_assault() {//function paladin_assault takes nothing returns nothing
			...
			if (assaultN <= 1) {//if assaultN <= 1 then
				set_assault(1, DemoAi.paladin);//call set_assault(1, paladin     )
				set_assault(4, DemoAi.footman);//call set_assault(4, footman     )
				set_assault(3, DemoAi.rifle);//call set_assault(3, rifle       )
			} else if (assaultN <= 2) {//elseif assaultN <= 2 then
				set_assault(1, DemoAi.paladin);//call set_assault(1, paladin     )
				set_assault(6, DemoAi.footman);//call set_assault(6, footman     )
				set_assault(6, DemoAi.rifle);//call set_assault(6, rifle       )
			}//endif
