	...
	public class N03_purpleAi {
		...
		private void lower_defcon() {//function lower_defcon takes nothing returns nothing
			...
			if (defcon == 4) {//if defcon == 4 then
				//
				AddGuardPost(CommonAi.NECRO, -1800, 230);//call AddGuardPost( NECRO,		-1800,  230 )
				CampaignDefenderEx(1, 1, 2, CommonAi.NECRO);//call CampaignDefenderEx( 1,1,2, NECRO       )
				//
			} else if (defcon == 3) {//elseif defcon == 3 then
				//
				AddGuardPost(CommonAi.GARGOYLE, -2320, 725);//call AddGuardPost( GARGOYLE,	-2320,  725 )
				AddGuardPost(CommonAi.NECRO, -2340, 325);//call AddGuardPost( NECRO,       -2340,  325 )
				...
				//
				AddGuardPost(CommonAi.ABOMINATION, -2110, 70);//call AddGuardPost( ABOMINATION, -2110,   70 )
				AddGuardPost(CommonAi.NECRO, -2620, 480);//call AddGuardPost( NECRO,       -2620,  480 )
				CampaignDefenderEx(1, 1, 2, CommonAi.ABOMINATION);//call CampaignDefenderEx( 1,1,2, ABOMINATION )
				//
			} else {//else
				//
				AddGuardPost(CommonAi.ABOMINATION, -2025, 570);//call AddGuardPost( ABOMINATION, -2025,  570 )
				AddGuardPost(CommonAi.GARGOYLE, -2450, 520);//call AddGuardPost( GARGOYLE,	-2450,  520 )
				AddGuardPost(CommonAi.CRYPT_FIEND, -2120, 290);//call AddGuardPost( CRYPT_FIEND, -2120,  290 )
				CampaignDefender(CommonAi.EASY, 1, CommonAi.TICHONDRIUS);//call CampaignDefender( EASY, 1, TICHONDRIUS )
				SetCaptainHome(CommonAi.DEFENSE_CAPTAIN, N03_purpleAi.FAILSAFE_X, N03_purpleAi.FAILSAFE_Y);//call SetCaptainHome(DEFENSE_CAPTAIN,FAILSAFE_X,FAILSAFE_Y)
			}//endif
		}//endfunction
		//
		//
		...
		private void rebuild_town() {//function rebuild_town takes nothing returns nothing
			...
			if (GetUnitCountDone(UNDEAD_ALTAR)>0) {//if GetUnitCountDone(UNDEAD_ALTAR)>0 then
				SetBuildUnit(1, CommonAi.TICHONDRIUS);//call SetBuildUnit( 1, TICHONDRIUS   )
			}//endif
			...
			if (TownCount(NECROPOLIS_1)==0 || GetUnitCount(UNDEAD_MINE)==0 || GetUnitCount(UNDEAD_ALTAR)==0  || GetUnitCount(CRYPT)==0) {//if TownCount(NECROPOLIS_1)==0 or GetUnitCount(UNDEAD_MINE)==0 or GetUnitCount(UNDEAD_ALTAR)==0  or GetUnitCount(CRYPT)==0 then
				ShiftTownSpot(N03_purpleAi.MAIN_TOWN_X, N03_purpleAi.MAIN_TOWN_Y);//call ShiftTownSpot(MAIN_TOWN_X,MAIN_TOWN_Y)
			...
			if (GetUnitCount(GRAVEYARD)==0) {//if GetUnitCount(GRAVEYARD)==0 then
				ShiftTownSpot(N03_purpleAi.GRAVEYARD_X, N03_purpleAi.GRAVEYARD_Y);//call ShiftTownSpot(GRAVEYARD_X,GRAVEYARD_Y)
				SetBuildUnit(1, CommonAi.GRAVEYARD);//call SetBuildUnit( 1, GRAVEYARD     )
				return;//return
			}//endif
			SetBuildUnit(CommonAi.campaign_wood_peons, CommonAi.GHOUL);//call SetBuildUnit( campaign_wood_peons, GHOUL )
			//
			//
			ShiftTownSpot(N03_purpleAi.MAIN_TOWN_X, N03_purpleAi.MAIN_TOWN_Y);//call ShiftTownSpot(MAIN_TOWN_X,MAIN_TOWN_Y)
			...
			SetBuildUnit(2, CommonAi.NECRO);//call SetBuildUnit( 2, NECRO             )
			SetBuildUnit(1, CommonAi.SLAUGHTERHOUSE);//call SetBuildUnit( 1, SLAUGHTERHOUSE    )
			SetBuildUnit(2, CommonAi.ABOMINATION);//call SetBuildUnit( 2, ABOMINATION       )
		...
		private void main() {//function main takes nothing returns nothing
			...
			SetCaptainHome(CommonAi.DEFENSE_CAPTAIN, N03_purpleAi.DEFENSE_X, N03_purpleAi.DEFENSE_Y);//call SetCaptainHome(DEFENSE_CAPTAIN,DEFENSE_X,DEFENSE_Y)
			CampaignDefenderEx(1, 1, 1, CommonAi.CRYPT_FIEND);//call CampaignDefenderEx( 1,1,1, CRYPT_FIEND )
			while (true) {//loop
