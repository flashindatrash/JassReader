...
namespace Jass {
	...
	public class H06x07Ai {
		...
		private void main() {//function main takes nothing returns nothing
			...
			SetCaptainHome(CommonAi.BOTH_CAPTAINS, 2167, 8137);//call SetCaptainHome(BOTH_CAPTAINS,2167,8137)
			CampaignDefenderEx(1, 1, 1, CommonAi.CHAOS_GRUNT);//call CampaignDefenderEx( 1,1,1, CHAOS_GRUNT     )
			CampaignDefenderEx(1, 1, 1, CommonAi.CHAOS_WARLOCK);//call CampaignDefenderEx( 1,1,1, CHAOS_WARLOCK   )
			CampaignDefenderEx(1, 1, 1, CommonAi.CHAOS_KODO);//call CampaignDefenderEx( 1,1,1, CHAOS_KODO      )
			CampaignDefenderEx(0, 0, 1, H06x07Ai.NERATHOR);//call CampaignDefenderEx( 0,0,1, NERATHOR        )
			...
			CampaignAttackerEx(0, 0, 1, H06x07Ai.NERATHOR);//call CampaignAttackerEx( 0,0,1, NERATHOR        )
			...
			while (true) {//loop
				...
				CampaignAttackerEx(1, 1, 1, H06x07Ai.NERATHOR);//call CampaignAttackerEx( 1,1,1, NERATHOR        )
				CampaignAttackerEx(1, 1, 2, CommonAi.CHAOS_WARLOCK);//call CampaignAttackerEx( 1,1,2, CHAOS_WARLOCK   )
				SuicideOnUnitsEx(CommonAi.M5, CommonAi.M5, CommonAi.M3, U08_tealAi.user);//call SuicideOnUnitsEx(M5,M5,M3,user)
				InitAssaultGroup();//call InitAssaultGroup()
				CampaignAttackerEx(3, 3, 5, CommonAi.CHAOS_GRUNT);//call CampaignAttackerEx( 3,3,5, CHAOS_GRUNT     )
